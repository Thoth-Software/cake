defmodule Caque.Jobs.DocumentIngestionJobTest do
  use Caque.ObanCase, async: true
  use Oban.Testing, repo: Caque.Repo

  import Mox
  require Logger

  alias Caque.Jobs.DocumentIngestionJob
  alias Caque.TestPipeline
  alias Caque.FailingTestPipeline
  alias Caque.Embeddings.Mock, as: EmbeddingsMock
  ...
  setup do
    ...
    Application.put_env(:caque, :embeddings_module, EmbeddingsMock)

    # Set Logger level to info for tests to capture all logs
    Logger.configure(level: :info)

    on_exit(fn ->
      Application.delete_env(:caque, :embeddings_module)
    end)
  ...
  describe "enqueue/1 with module atom" do
    test "successfully enqueues a job with a pipeline module atom" do
      ...
      assert job.args["source_pipeline"] == "Caque.TestPipeline"
    ...
    test "converts Elixir module prefix correctly" do
      ...
      assert {:ok, %Oban.Job{} = job} = DocumentIngestionJob.enqueue(args)
      assert job.args["source_pipeline"] == "Caque.Documents.Hexdocs.Pipeline"
    end
  end
  ...
  describe "enqueue/1 with string module name" do
    test "successfully enqueues a job with a string pipeline module name" do
      args = %{
        source_pipeline: "Caque.TestPipeline",
        embedding_service: :openai,
        version: %{major: 1, minor: 18, patch: 3},
        embedding_model: "text-embedding-ada-002"
      }
  ...
  describe "perform/1 - unit tests with mocks" do
    ...
    test "successfully performs the ingestion pipeline" do
      ...
      assert log =~ "Starting document ingestion with Caque.TestPipeline"
    ...
    test "logs error when pipeline fails" do
      ...
      args = %{
        "source_pipeline" => "Caque.FailingTestPipeline",
        "embedding_service" => "openai",
        "version" => %{"major" => 1, "minor" => 18, "patch" => 3},
        "embedding_model" => "text-embedding-ada-002"
      }
  ...
  describe "integration tests with Oban" do
    ...
    test "job is properly enqueued and can be executed" do
      ...
      assert job.queue == "default"
      assert job.worker == "Caque.Jobs.DocumentIngestionJob"
    end
    ...
    test "failed jobs can be retried" do
      ...
      fresh_job = Caque.Repo.get(Oban.Job, job.id)
    ...
    test "job args are properly serialized and deserialized" do
      ...
      retrieved_job = Caque.Repo.get(Oban.Job, job.id)

      # Verify all fields are correctly serialized
      assert retrieved_job.args["source_pipeline"] == "Caque.TestPipeline"
  ...
  describe "error handling" do
    ...
    test "handles non-existent pipeline module gracefully" do
      args = %{
        "source_pipeline" => "Caque.NonExistentPipeline",
        "embedding_service" => "openai",
        "version" => %{"major" => 1, "minor" => 18, "patch" => 3},
        "embedding_model" => "text-embedding-ada-002"
      }
