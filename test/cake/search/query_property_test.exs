defmodule Cake.Search.QueryPropertyTest do
  ...
  # This reflects OpenSearch's bool query semantics: KNN (vector similarity) is a
  # hard requirement so it goes in must; keyword matching is a relevance signal
  # that boosts score but isn't required so it goes in should; and term filtering
  # is a hard constraint that bypasses scoring entirely so it goes in filter.
