defmodule Cake.Conversation.StateTest do
  ...
  fields (`:max_context_tokens`, `:max_self_ask_iterations`,
  `:max_ircot_iterations`) are enforced keys with no struct defaults.
  `Conversation.build_state/1` is the sole constructor and always
  supplies them from opts or config, so a hand-built `State` that forgot
  one must fail loudly rather than carry a copy of the config default
  that could silently drift from `config.exs`.
  """
  ...
  test "constructs with every enforced key supplied" do
    ...
    assert state.max_ircot_iterations == 5
  end
