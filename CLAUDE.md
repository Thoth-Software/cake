...
- **kNN `ef_search` rejected by OpenSearch (#245 finding):** `Cake.Search.Query.knn/5` emits `ef_search` as a top-level key of the knn clause and `Cake.Search` sets it on every `:vector`/`:hybrid` query, but OpenSearch (3.2.0 verified) rejects it — `[knn] unknown field [ef_search]` — so those searches fail; the accepted form is `method_parameters: %{ef_search: n}`. The conformance test "search/1 accepts the vector query Cake.Search builds" stays red until the encoding is fixed. Not silently resolved here: the fix changes `Query.knn/5`'s output and the unit tests that pin it.
