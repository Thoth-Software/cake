...
- **Real-cluster conversation turns → `Cake.ConversationIntegrationHelpers`.** `test/support/conversation_integration_helpers.ex` seeds a `Chunk` corpus with fixed vectors into the test's collection (`seed_standard_corpus/1` as a named setup, or `seed_corpus!/2`), binds a `Cake.GDS` to it (`CollectionGDS`, configured per test — so suites on it are `async: false`), scripts generation through `Req.Test` and the real OpenAI transport (`script_generation!/1`), and starts or attaches to a conversation with every collaborator allowed for its turn tasks. Tier-1 suites `use Cake.SearchIntegrationCase`; the live tier (`Cake.ConversationLiveTest`) sits on `Cake.LiveLLMCase` with `setup :integration_collection` and the live helpers (CLAUDE.md "Live LLM tests").
