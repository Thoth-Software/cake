...
- **Real S3-compatible store → `Cake.S3IntegrationCase`.** `test/support/s3_integration_case.ex` tags the module `:integration`, points ExAws's `:s3` service at `S3_ENDPOINT_URL` with a fixed key pair for each test, hands it a bucket of its own as `:bucket` (also set as `:book_storage_s3_bucket`, so `Cake.Books.Adapters.S3` needs no config of its own), exposes `bucket_exists?/1` / `object_keys!/1` / `unique_bucket_name/1` for observing the adapter from outside, drops every bucket under the test's prefix afterwards, and refuses `async: true`. `configure_s3!/1` with overrides (a closed port, `retries: [max_attempts: 1]`) provokes a request error. Unit tests keep using `Cake.Books.Adapters.Mock`.
