defmodule Cake.Jobs.DocumentIngestionJob do
  @moduledoc """
  ...
  `Cake.Documents.Pipeline` behaviour.
  ...
  1. Create a module under `Cake.Documents.YourSource.Pipeline`
  2. Implement the `@behaviour Cake.Documents.Pipeline` callbacks
  3. Enqueue a job with your pipeline module

  Example with a hypothetical Clojure docs source:

      DocumentIngestionJob.enqueue_for_version(
        Cake.Documents.Clojuredocs.Pipeline,
        :openai,
        {1, 11, 1},
        "text-embedding-ada-002"
      )
  ...
  ## Parameters
    ...
    * `source_pipeline` - Module atom implementing `Cake.Documents.Pipeline` behaviour
    * `embedding_service` - Atom for the embedding service (e.g., :openai)
  ...
  ## Examples
      ...
      iex> alias Cake.Documents.Hexdocs.Pipeline, as: HexdocsPipeline
